#!/usr/bin/env python3
"""
Spotify Now Playing Widget - Advanced UI Implementation

This Flask app serves a dynamic SVG widget showing the currently playing
or recently played Spotify track with advanced features like color palette
extraction and customizable themes.

Usage:
1. Set up your Spotify credentials in .env
2. Run: python api/spotify.py
3. Visit: http://localhost:5000 to see the widget
4. Use the SVG URL in your README: ![Spotify](http://localhost:5000)

For production deployment on Vercel, the app will be available at your domain.
"""
...
def makeSVG(data, bg, border):
    ...
    try:
        if data.get("is_playing"):
            ...
            recent = get(RECENTLY_PLAYING_URL)
            items = recent.get("items", [])
            if not items:
                raise Exception("No recently played tracks found.")
            item = random.choice(items)["track"]
    except Exception as e:
        print(f"⚠️ Spotify returned no items: {e}")
        # fallback placeholder
        item = {
            "name": "Nothing playing",
            "artists": [{"name": "Silence"}],
            "external_urls": {"spotify": "https://open.spotify.com/"},
            "album": {"images": []},
        }

    img_url = item["album"]["images"][1]["url"] if item["album"]["images"] else PLACEHOLDER_URL
    image = loadImageB64(img_url)
    barPalette = gradientGen(img_url, 4)
    songPalette = gradientGen(img_url, 2)
    ...
    return render_template(getTemplate(), **ctx)
